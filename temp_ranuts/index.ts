...
import { onCreateNew, openDocumentFromUrl, openDocumentFromBase64, setUICallbacks } from './lib/document';
import {
  createControlPanel,
...
declare global {
  interface Window {
    ...
    openDocumentFromBase64?: (base64Str: string, fileName: string) => Promise<void>;
    DocsAPI: {
...
window.openDocumentFromBase64 = openDocumentFromBase64;
...
const { file, src, name } = getAllQueryString();
...
if (documentUrl) {
  ...
  try {
    ...
    openDocumentFromUrl(decodedUrl, name);
  } catch (error) {
    // If decoding fails, try using original URL
    console.warn('Failed to decode URL, using original:', error);
    openDocumentFromUrl(documentUrl, name);
  }
}

// Add message listener for base64 document loading
let hasOpenedDocument = false;
window.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'loadDocumentBase64') {
    if (!hasOpenedDocument && window.openDocumentFromBase64) {
      hasOpenedDocument = true;
      console.log('[ranuts-document] Calling openDocumentFromBase64 for:', event.data.name);
      
      // Reply to parent that we got it
      if (event.source) {
        event.source.postMessage({ type: 'documentLoadedACK' }, { targetOrigin: '*' });
      }

      window.openDocumentFromBase64(event.data.base64, event.data.name).catch(e => {
        hasOpenedDocument = false; // Reset if failed
        console.error('[ranuts-document] openDocumentFromBase64 failed:', e);
      });
    }
  }
});
...
if ('serviceWorker' in navigator) {
  if (window.location.protocol !== 'file:') {
    ...
    // If we are on file: protocol, unregister any existing service workers
    navigator.serviceWorker.getRegistrations().then(registrations => {
      for (const registration of registrations) {
        registration.unregister().then(boolean => {
          console.log('Unregistered service worker on file: protocol:', boolean);
        });
      }
    });
  }
