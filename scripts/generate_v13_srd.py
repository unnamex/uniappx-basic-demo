"""
基于 V12 SRD 包，为所有 table/tableTree 组件的 fields 添加 minWidth 配置。
使列宽分配更合理，防止 flex 列被压缩到不可读。

规则：
  - 固定宽度列（width > 0）：若 width < 100，设 minWidth = width（保底）
  - 弹性列（width == -1 或缺失）：根据列类型自动设置合理的 minWidth
  - richText 列：minWidth = 100（只显示展开按钮，不需要太宽）
  - 普通文本弹性列：minWidth = 120
"""
...
def add_min_width_to_fields(fields):
    """为 fields 数组中的每个字段添加合理的 minWidth"""
    modified = False
    for field in fields:
        # 跳过已经配置了 minWidth 的字段
        ...
        if width > 0:
            # 固定宽度列：如果宽度小于 100，设 minWidth = width 保底
            if width < 100:
                field['minWidth'] = width
                modified = True
        else:
            # 弹性列（flex）：根据类型设置 minWidth
...
def main():
    ...
    in_path = os.path.join(project_root, 'V8发动机装配工艺_V12_encrypted.srd')
    out_path = os.path.join(project_root, 'V8发动机装配工艺_V13_encrypted.srd')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_data), 'r') as zin, \
        ...
        for item in zin.infolist():
            ...
            if item.filename.endswith('manifest.json'):
                ...
                data['version'] = '13.0'
                data['description'] = 'V13: 为 table/tableTree 组件的列配置添加 minWidth 最小宽度保障'
                content = json.dumps(data, ensure_ascii=False, indent=2).encode('utf-8')
                print("  ✓ manifest.json 版本号更新为 13.0")
